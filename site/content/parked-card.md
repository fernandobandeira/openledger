...
[`schema.sql`](/source/card-schema) — the authorization, hold and clearing half of the old
...
Its design is [ADR-0008](/decisions/0008-authorization-holds), which is unaffected —
...
[ADR-0008 §*Known, and not fixed*](/decisions/0008-authorization-holds#known-and-not-fixed)
...
written into [ADR-0009](/decisions/0009-module-boundaries).

**An earlier version of this section gave a reason that does not survive its own evidence.** It
blamed ADR-0008's open findings directly — "shipping DDL whose known defects are recorded and unfixed
is how a schema accumulates state nobody can safely change later". But those findings were recorded
before the split and nothing about them changed on the day of it; and ADR-0008's own header says the
...
[ADR-0009's alternative (a)](/decisions/0009-module-boundaries) says so.

**And parking does not deliver what [ADR-0009](/decisions/0009-module-boundaries)
...
1. **[ADR-0009](/decisions/0009-module-boundaries) gets applied** — these objects move
...
4. **The credit line it decides against gets designed.** The authorization path reads
   ...
   spike artefact that never graduated, and [spike 010](/spikes/010-go-or-rust) had to
