...
**Status:** closed. Feeds [ADR-0007](/decisions/0007-schema-conventions-and-chart)
and M0/M1.
...
Fixed by `completeness.sql`: every account type carries a `NOT NULL`
...
deferred constraint trigger from [spike 002](/spikes/002-sqlc-vs-jet). It fires at `COMMIT`,
...
`chart.sql` adds a real chart of accounts as data:
...
The card chart in `chart.sql` is **seed data for the reference product**, not part
of the engine. A marketplace ships a different one against the same core.

## Proof: the golden trace

`golden_trace.sql` posts the full the reference product spec lifecycle;
`verify.sql` evaluates the equation after each transaction.
...
[ADR-0006](/decisions/0006-time-and-as-of) demonstrating itself by accident, on a
...
the complete set?* — which is what `completeness.sql` answers.
...
[ADR-0006](/decisions/0006-time-and-as-of) did for `effective_at`. It is also the
...
security`, on PG 18.6. [Spike 003](/spikes/003-throughput-ceiling) found `CopyFrom` load-bearing on its
coalesced-batching path (this line credited spike 002, which does not use it), and
[spike 003](/spikes/003-throughput-ceiling) uses it for the coalesced batch worth 4.4×.
