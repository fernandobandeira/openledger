...
quality — a premise [card 0001 · authorization holds](/card/decisions/0001-authorization-holds) removed when it chose River.
...
### 1. [0001](/decisions/0001-rust-and-postgres) recommended the trap it rejected go-jet for
...
[0001](/decisions/0001-rust-and-postgres), which now carries the finding.
**`Lax` is one switch that makes every money field on a struct silently zeroable.**

### 2. [0005](/decisions/0005-event-log-and-write-path)'s headline claim was false in Go

The write-path ADR, now [0005](/decisions/0005-event-log-and-write-path): *"An
...
'financial_authorization'` (a real gap — [card 0001 · authorization holds](/card/decisions/0001-authorization-holds)
...
[0007](/decisions/0007-schema-conventions-and-chart) builds
...
[0003](/decisions/0003-migrations)'s criterion, reproduced: 2,000,000 rows,
...
**How much this should weigh is less than it first appears.** [0003](/decisions/0003-migrations)
...
[0003](/decisions/0003-migrations) rejected. The lock is a guard against a botched
...
harnesses were scratch and are not kept — [0004](/decisions/0004-where-logic-lives) is
