# Spike 002 — what card processors and ledger APIs actually do
...
ADR-0001 says **an authorization writes no ledger entry; only a clearing posts.**
...
## Delta versus cumulative total — the question ADR-0001 was right to refuse to answer
...
| **Galileo** | **both, in different fields** | *"SoFi Tech Solutions immediately combines the incremental amount with the previous amount and presents the **cumulative amount** as the transaction amount in external data — Auth API webhook, Authorized Transactions RDF, the Events API messages, and Program API responses. **The incremental amount will be present in the local amount fields.**"* — an earlier reading of this row truncated the quote mid-passage and filed Galileo as cumulative-only — [authorization](https://docs.tech.sofi.com/pro/docs/authorization) |
...
- **Stripe's two-event-ids caveat, which ADR-0001 already cites, is real:** *"In some cases, two
