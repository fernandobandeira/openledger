...
**The mistakes that hurt a ledger most are the ones that pass every check. This decision is a set of
conventions chosen to make each of those mistakes loud, plus a chart of accounts kept as data the
engine reads rather than logic baked into it.** Two kinds of silent mistake:

- **A schema change breaks something and the build stays green.** A migration drops a column and the
  database silently drops the two indexes that were built on it; a report read has been a scan-and-sort
  ever since, unnoticed. An index gets a name that names the wrong table, so it is built on the wrong
  thing and nothing complains. Nothing was comparing the schema against what it is supposed to be, so
  nothing caught it.
- **A report loses a whole chunk and still balances.** Balancing proves the two sides of the books
  agree; it does not prove the report listed everything. Drop one account and one side of the equation
  comes up short and the check catches it — but drop a whole tenant, or a whole currency, or a whole
  date range, and what fell out was itself balanced, so the books still tie out while the report is
  quietly missing a slice.

The fix for the first is conventions that turn a silent slip into a failed build — chief among them a
CI test that dumps the whole schema and diffs it against a committed snapshot. The fix for the second
is to build every report by starting from the chart of accounts and enumerating outward, so an account
with no activity shows up as a zero instead of vanishing — and to keep the chart itself in a table
(which account types exist, what each rolls up to on a statement) rather than hard-coding it, so a
different business can ship its own chart as seed data.

### Conventions

| | |
| --- | --- |
| **1. Name every object `<prefix>_<table>__<what>`** | Prefix one of `pk_ uq_ ix_ ck_ fk_ ex_ rls_` — an exclusion constraint and a row-level-security policy are named objects too ([0011](/decisions/0011-period-close-and-report-axes), [0013](/decisions/0013-write-path-contract)); the double underscore survives table names that already contain underscores. `<what>` is a **rule name**, not a column list — `ix_entries__asof_commit`, `uq_txn__one_supersession`, `uq_entries__account_seq` — because a rule name does not go stale when a column joins the index. `<table>` is usually abbreviated (`entries`, `txn`, `balances`), since an index name is capped at 63 characters. |
| **2. A schema snapshot test in CI** | Apply the migrations to an empty database, dump every index, constraint, trigger **and every trigger's and event trigger's enablement state** (`pg_trigger.tgenabled`, `pg_event_trigger.evtenabled` — the `pg_dump --disable-triggers` restore path downgrades `ENABLE ALWAYS` to `ENABLE ORIGIN` and that column is the only place it shows, [0009](/decisions/0009-append-only-perimeter)), **every policy** (`pg_policy`) **and every view's `reloptions`** (a `security_invoker` flipped to `false` is a tenant leak no query fails on, [0013](/decisions/0013-write-path-contract)). **[0009](/decisions/0009-append-only-perimeter)'s owner-accident DDL class is out-of-model and this snapshot is its only backstop, so the charge is widened to catch that whole class**: also dump **`pg_class.relrowsecurity` and `relforcerowsecurity`** (a `DISABLE ROW LEVEL SECURITY` flips only this and is invisible otherwise), **`pg_get_viewdef` for every view** (a `CREATE OR REPLACE VIEW reconciliation AS SELECT 0`, a dropped report view, or a `RENAME`/`SET SCHEMA` that moves a base table all show here), **`account_types.is_perimeter` and `mirror_type`** (an identity `UPDATE` silences a perimeter or mirror check), **the full `pg_constraint` set — names and definitions** (a `DROP CONSTRAINT` or `DROP INDEX` on a protected table shows as a missing row), and **every object and column comment (`pg_description`)** — so a comment removed or drifted from the catalog (convention 16) shows in the diff like any other schema change. And — separately — every `NOT VALID` constraint; diff against a committed snapshot. **This is the highest-leverage item here.** *(**Amended 2026-09-01 by [0020](/decisions/0020-checkpoint-on-the-report-path): the widened charge does not yet catch its whole class.** The dump records `relkind`, so *converting* a table to partitioned is visible — but `relpartbound`, `relispartition`, `pg_inherits`, `pg_partitioned_table` and `pg_get_partkeydef` appear nowhere, so a `DETACH PARTITION` removes rows from every reader with the relations section byte-identical. The nine trigger-bearing tables would still show drift, because `DETACH` drops triggers cloned from the parent; everything else in this schema would not. Those five catalogs join the dump in the migration that first partitions anything.)* |
...
| **16. Comment every table and load-bearing column in the *catalog*, not just the file** | `COMMENT ON` every table, every load-bearing column, and the report views and functions, so the **live** schema documents itself: a client, an ORM introspector, or an AI reading `\d+`, `obj_description()`/`col_description()` or `information_schema` reads the intent straight from the catalog, without opening the migration. The comments are **distilled and navigational** — what the object is plus the one gotcha — and cite the ADR inline; the rationale stays in the ADRs, not pasted into the string. This is **especially for AI navigability of the schema** — the querying agent that never sees the repo. Convention 2's snapshot dump includes `pg_description`, so a removed or drifted comment fails the build like any other schema change. |
...
| **11. Completeness is a separate invariant from balance** | The accounting equation does not prove a report enumerated everything, so reports enumerate **from the chart outward**: `balance_sheet_at` starts `FROM fs_lines` and left-joins the numbers on, so a line with no activity appears as a zero instead of vanishing. Every presentable account type has a `chart_presentation` row naming its line at each chart version ([0012](/decisions/0012-chart-governance)), so there is always something to enumerate — the statement line left `account_types` for the append-only `chart_presentation` when a reclassification became a new chart version. |
| **12. The equation is evaluated per currency** | And a currency-blind one is vacuous — below. |
| **13. Where one logical account is split across rows, summing them is netting, and netting has rules** | Each account type declares a `counterparty_scope`; where the split key *is* the counterparty, opposite-sign members must be presented gross rather than netted. **Now a mechanism, not a declaration**: the balance sheet evaluates the position per account and routes an opposite-sign one to its declared contra line, and `ck_accounts__per_shard_is_owned` refuses the house account that would have netted at write time ([0012](/decisions/0012-chart-governance)). |
| **14. Every report reads `status = 'posted'` and nothing else** | All three statements — the `trial_balance` view and the `balance_sheet_at`/`income_statement_for` functions — join `ledger_transactions … AND x.status = 'posted'`. A `pending` transaction is a claim about what may happen; a statement reports what did. Without the filter a pending authorization was recognised as revenue and its posted resolution counted it again — **500.00 of interchange, twice**. |
...
| **CI needs the snapshot-diff step — built 2026-08-31** | It is worth more than any test we would write by hand, and convention 2 is now enforced: the e2e suite's `schema_snapshot.rs` migrates an empty scratch database with the compiled binary, dumps the charged catalog state as deterministic text, and diffs it against the committed `schema/snapshot.txt` on every push (`make schema-snapshot-check`; regeneration is the explicit opt-in `make schema-snapshot`, so a normal run can only fail on drift). One aggregation the charge did not foresee: the internal foreign-key trigger names are OID-derived ([0009](/decisions/0009-append-only-perimeter) §A), so their enablement is dumped per constraint rather than per trigger name. |
...
| **PostgreSQL 18 adds catalog rows nobody wrote** | Verified against `migrations/00001_baseline.sql`: **every index and every named constraint matches `^(pk\|uq\|ix\|ck\|fk)_`**, and no object carries a PostgreSQL default name. But PG18 materialises `NOT NULL` as a real catalog constraint, so `pg_constraint` lists **104** server-generated rows named `<table>_<column>_not_null` on the merged baseline (142 with the parked card DDL loaded as well). They cannot be supplied in `CREATE TABLE`, and nothing references them. |
| **Reclassification is blocked while accounts exist — in one direction only** | `fk_accounts__type` refuses a change to a type's `category` or `normal_balance` while accounts reference it. It does **not** refuse a move of `fs_line` to another line of the same statement and side: verified, `fbo_cash` moved from `restricted_cash` to `cash` and 440.00 of customer float silently became unrestricted liquidity on an already-issued balance sheet — the exact harm `schema/chart.sql` cites Reg S-X 5-02.1 and ASC 230-10-45-4 for. **Closed by [0012](/decisions/0012-chart-governance)**: `fs_line` left `account_types` for an append-only `chart_presentation` keyed by chart version, so the move is refused in every direction and a reclassification is a new chart version with IAS 1.41's disclosure attached. The blocked direction was worth blocking: changing a type's category silently rewrote the income statement, revenue 9.00 → 0.00 on the golden trace with every check green, because reports join through the type while the guard only fired on the account. |
| **`trial_balance` still enumerates inward** | And is the wrong thing to build a completeness claim on. `balance_sheet_at` enumerates chart-outward (`FROM fs_lines`), and the accounting-equation check `recon_equation_breaks` foots over *its* output, so completeness is carried by the chart-outward form; `trial_balance` (the per-account gross substrate) has not been rebuilt that way and is not where the completeness claim lives. |
