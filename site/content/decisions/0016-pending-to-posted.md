...
`uq_txn__one_resolution` allows the target exactly one, and the reconciliation views already name
...
| What may a resolution target? | **A pending, unresolved transaction on the caller's own tenant — enforced by the writer.** The foreign key holds existence and nothing declarative holds the semantics: ADR-0004 reproduced a posted transaction "resolved" by another posted one taking revenue to **−49,223** with drift at 0. The single-call statement reads the target and gates the transaction insert on `status = 'pending'` and no existing resolution; a failed gate comes back as a diagnosis and the service refuses by name — `unknown_resolve_target`, `resolve_target_not_pending`, `already_resolved`, each a 422 with nothing written | [ADR-0004](/decisions/0004-where-logic-lives); the gate is Rust-side logic in the adapter's one statement, **no trigger** |
| Two resolutions racing one target? | The gate reads committed state, so the race falls through to **`uq_txn__one_resolution`** — the loser blocks on the winner's uncommitted index tuple and, when it commits, is refused; the adapter maps that one constraint by name onto the same `already_resolved` refusal the sequential case gets, never a 500 | the baseline's partial unique index as backstop; the mapping is pinned by the **uncommitted-rival** e2e test, which holds a rival resolution open on a second connection until the API call is provably blocked on it (polled in `pg_stat_activity`), then commits — the eight-racer test holds the *gate* under concurrency, whose losers typically arrive after the winner commits |
...
| **A `voids` / expiry surface in the same change** | Deferred out of the resolution slice — and since decided: the void is the zero-posting reversal in [Reversals and the void](#reversals-and-the-void--proposed), shipping with the reversal slice. Expiry stays M8's. |

## What it costs

- **A pending transaction that will never resolve stays pending until the reversal slice lands.**
  Voiding is designed — the zero-posting reversal in
  [Reversals and the void](#reversals-and-the-void--proposed), ratified 2026-08-31 — and NOT built;
  hold expiry stays M8's durable timers. Until the slice ships, the bridge's
  `oldest_pending_effective_at` is the aging alarm ADR-0010 put there for exactly this.
...
- **The race refusal hangs on a constraint name.** The adapter maps `uq_txn__one_resolution` by
  its catalog name; rename the index — or the match string — and the genuinely-blocked case
  silently degrades to a 500. The **uncommitted-rival** e2e test is what fails when that happens
  (proven by mutation: renaming the string fails that test and nothing else); the eight-racer test
  cannot catch it, because its losers arrive after the winner commits and are refused by the gate.
- **The gate's `already_resolved` reads `resolves_id` only, while the schema retires a pending on
  resolved OR reversed** (`recon_pending_bridge`, the `superseded` bucket). Unreachable today —
  nothing writes `reverses_id` — and now owned:
  [Reversals and the void](#reversals-and-the-void--proposed) widens the gate to both pointers in
  the reversal slice (a reversed pending must refuse a resolution, and the supersession index
  below is the backstop for the resolve-vs-reverse race). Until that slice ships, the coupling
  stays named beside the gate's SQL.
- **The endpoint's 422 family grew from three types to six.** Each is real and reachable
...
## Reversals and the void — ratified 2026-08-31, not built

**Status of this section: proposed, like the rest of this ADR — ratified by Fernando on
2026-08-31 after an adversarial review with prior-art research, and NOT built.** The rulings below
are the reversal slice's contract; where the review produced a worked failure mode, it is cited
the way the rest of this file cites spike measurements, and the three changes the design forces on
existing machinery are recorded as **implementation requirements** — the slice is not done until
each one holds.
...
1. **`recon_transaction_breaks` flags `no_entries`** — the ADR-0004 `TRUNCATE` scar's own class —
   ...
   That is a **new migration replacing the view** (the migration set is append-only: 00001 and
   00002 stay frozen), and the carve-out must be exactly that narrow: an entryless transaction
   with no `reverses_id`, or one whose target is posted, stays a break.
2. **ADR-0005's primitive grows a stated exception.** "A transaction is a list of postings" is that
   ADR's load-bearing sentence; the void is a transaction that is deliberately not one. The
   exception must be recorded in ADR-0005 itself when the slice lands, not discovered in the
   schema.
3. **`claim_and_append`'s answer must anchor on the claimed row before this ships.** The
   adversary's failure mode #1, recorded as a requirement: the statement's final `SELECT` answers
   one row per delta (`CROSS JOIN delta`), and a zero-entry void has **zero deltas** — so a
   successful void returns zero rows, which is today's replay signal. Unchanged, the first void
   would claim its key, write its marker, and then be answered as a replay of itself (statement B
   inside the same open transaction finds the uncommitted claim, hash matching). The return shape
   must anchor on `claimed` — rows back whenever the claim inserted, deltas or none.
...
one pointer, so the expression names the sole target. This subsumes both existing indexes and
lands as a new migration (the set is append-only); the adapter's constraint-name mapping and the
uncommitted-rival e2e pin extend to the new name — the mutation discipline above applies verbatim,
one name over.
