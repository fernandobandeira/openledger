...
| | |
| --- | --- |
| `recon_balance_breaks` | The balance cache against the journal, **per `(tenant_id, account_id, currency, stripe)`** — the cache row's own primary key, so a striped account is checked one stripe at a time and a lazily created stripe reconciles clean on its first write ([ADR-0018](/decisions/0018-batching-and-stripe-selection) §1). *(This cell read "per tenant, account and currency", omitting the stripe.)* In **six** classes: `balance_drift`, `seq_ahead`, `seq_behind`, `seq_gap`, `no_cache_row`, `no_entries` |
...
| **No check reads `ledger_events`, so the sweep is structurally blind to an orphaned event** | Added 2026-09-01, found by [ADR-0018 §3](/decisions/0018-batching-and-stripe-selection) and belonging here rather than only there. All ten checks read `ledger_entries`, `ledger_transactions`, `ledger_account_balances` and the report views; **not one reads the idempotency-claim table.** So an event row committed with no transaction joined to it — a burned idempotency key — is invisible to `SELECT * FROM reconciliation` **by construction**, not by oversight in a predicate. It is not hypothetical: the first batched writer gated a refused member *below* its key claim, leaving exactly that row, and the caller's retry was then answered `transaction_id: null, replayed: true` — which [0013](/decisions/0013-write-path-contract) says is the legitimate shape for the majority of accepted operations. **A refusal became indistinguishable from success, permanently, and the oracle reported ten zeros on the book carrying it.** It was found by asserting the event count directly. The consequence for anyone extending this family: **any claim about per-member or per-request isolation has to be tested against the transaction count, never against this sweep.** Whether an eleventh check should read `ledger_events` is open; the cheap form — events with no transaction and no replay — has a real false-positive population (an accepted operation that writes no transaction is legitimate), which is why it is recorded as a limitation rather than added in haste |
