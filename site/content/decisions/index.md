...
Twenty-five, each one file, each stating the decision first and then its evidence, its alternatives and
...
| [0019](/decisions/0019-read-path) | **The read path is a second inbound port on its own pool and its own login, and the cursor is validated in Rust before it reaches SQL** — five GET routes, `chart_version` always explicit, and reproducible *amounts* rather than a reproducible row set | A login that is a member of both the writer and reader roles **reads every tenant**: RLS policies are permissive and OR'd, and `pg_has_role` decides which apply, so the writer's `USING (true)` unions with the reader's tenant qual. And the database cannot refuse a bad cursor — `-1::xid8` wraps to `18446744073709551615` and returns the entire unpinned book looking correct, while NULL fabricates an all-zero *balanced* statement | accepted (ruled 2026-09-01) |
| [0020](/decisions/0020-checkpoint-on-the-report-path) | **One function reads the checkpoint, not three; the close is admitted to it by transaction identity rather than by a cursor; and the bounded drift check ships with a close-ordering invariant** | At-close and a reproducible cursor are **mutually exclusive in one transaction** — equality breaks under any concurrent writer and `pg_snapshot_xmin` silently loses a real posting — so 0011 §3's at-close sentence was unsatisfiable, not merely unbuilt. And "they scan from inception" was true of `balance_sheet_at` alone: the other two are already window-bounded, and the flow statement reports exactly the accounts a close zeroes | accepted (ruled 2026-09-01) |
...
| [0025](/decisions/0025-shortfalls-and-balance-floors) | **A shortfall is booked as a receivable, not as a negative balance — and that is what makes an account-level floor possible at all** | A liability in a debit position is economically an asset, and you cannot hold an allowance against it: a negative wallet is an implicit receivable, unaged and unreserved. And "no withdrawals below zero, but allow this chargeback" is a rule about the *operation*, which needs a DSL this project refuses — book the shortfall explicitly and the wallet never needs to go below zero, so a floor never needs an exception | accepted (the modelling rule); the floor is designed and **unbuilt** |
