...
**Status:** accepted — the `card`-schema decision stands; the symmetric `core` schema from the original
proposal is dropped.
**Artifact:** deferred — the card DDL is parked in `parked/card/`, applied by no migration and loaded by
CI on top of the core on every push, so the dependency is asserted rather than pasted; un-parking is
migration 00002 at [M7](/roadmap#m7--cards).
**Evidence:** [spike 008](/spikes/008-optional-modules).

## The decision

**OpenLedger has an optional card-issuing feature, and we want it cleanly separable from the core
ledger — installable and, above all, removable without a trace.** The way we get that: the card's
database tables live in their own **schema** — a named namespace inside one PostgreSQL database — called
`card`, kept apart from the core tables, which sit in the default `public` schema. That `card` schema is
created and changed by the one **migration runner** — the tool that applies ordered schema-change
scripts in sequence ([0003](/decisions/0003-migrations)) — that everything else uses, holding one lock
in one order. So the whole module drops in a single statement: `DROP SCHEMA card CASCADE`. It lands as
**migration 00002 when the card is un-parked ([M7](/roadmap#m7--cards))**.

The card feature is genuinely separable because no **foreign key** — a database link forcing one row to
reference a valid row in another table — crosses the card/core seam, so the two really can be pulled
apart. The instinct is then to give the card its own migration history too — but two independent
histories have no defined order between them, and a ledger needs its schema changes to happen in one
known sequence. So the card gets its own schema while staying on the single ordered list of migrations.
The seven core tables stay in `public` — only the card gets a schema of its own (the symmetric `core`
schema from the original proposal is dropped, because `migrations/00001_baseline.sql` is applied and
[0003](/decisions/0003-migrations) forbids editing it, so un-parking creates `card` in a new migration
00002 and leaves the core where it is).

- Against PostgreSQL 18.6 the split lands **4 tables / 2 views / 10 indexes** in the `card` schema,
  beside the **7 tables / 3 views / 23 indexes** of the core that stay in `public`. *Those six numbers
  are quoted from [spike 008](/spikes/008-optional-modules)'s own measurement of the split it built,
  which is why they appear here rather than only in
  [the database page](/database#what-the-schema-enforces-today) — that section is the single canonical
  census of the shipped schema, and if the two ever disagree, it is right and this is a stale
  quotation.*
- **The card module becomes removable.** `DROP SCHEMA card CASCADE` removes it in one statement —
  verified to leave all seven core tables, all nine core foreign keys and all three report views intact.
...
- A committed test asserts **no relation name is defined in both schemas**, because `search_path`
  resolves a collision silently to the card one.

## The evidence

Read from `pg_constraint`, **not one foreign key crosses the card/core boundary in either
direction** — the only card FK is
...
| | Why not |
| --- | --- |
| **(a) One set, card tables in `public`** | What we *had* before the card DDL was parked. Costs a wallet user 4 tables, 2 views, 10 indexes and 1 enum they never write to — which is cheap — and costs [0002](/decisions/0002-scaling) a claim that is false, which is not. This decision keeps (a)'s machinery and fixes only the claim. |
...
- **`migrations/00001_baseline.sql` hardcodes `public` twice**, in the `GRANT USAGE` and the trailing
  `REVOKE CREATE`, and
  [0007](/decisions/0007-schema-conventions-and-chart)'s snapshot test now covers two schemas.
