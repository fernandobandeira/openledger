//! Spike 021 DRIFT VARIANT D1 (see drift/RUN-drift.sh). ADR-0013's write-path contract expressed with `utoipa` 5.5.0 + `utoipa-axum` 0.2.0.
//!
...
#[derive(OpenApi)]
#[openapi(
    paths(create_transaction, get_account_balance),
    info(
...
// D1: the routes are declared HERE and the paths are declared AGAIN in the
// `#[utoipa::path]` attributes above. Both of these disagree with the
// annotations, and nothing complains.
fn router() -> Router {
    Router::new()
        .route("/v1/txns", post(create_transaction))
        .route("/v1/accounts/{account_id}/balance", get(get_account_balance))
}
