...
Eight, down from fourteen — consolidated by subject once the language changed. Two files were
...
| **The card tables ship inside the core baseline** | `schema/schema.sql` becomes migration 00001 ([0003](./0003-migrations.md), "there is exactly one copy of the schema"), and it contains `card_auth_events`, `card_auth_event_group`, `card_hold_groups` and two card views — **3 of 11 tables and 2 of 5 views**. So a marketplace-wallet user who never issues a card gets them anyway, while [0002](./0002-scaling.md) says the product layer is a plug-in and [0008](./0008-authorization-holds.md) is scoped as future work. **Those three cannot all be true.** Spike 011 is open on how comparable systems ship an optional module that owns tables, and what it does to migrations. |
