...
| **The auth hot path reads tables the schema does not define** | The design board's §03 decision reads `credit_lines`; `schema/schema.sql` defines no `credit_lines`, no `spend_controls` and no `card_holds`. [Spike 010](../../spikes/010-go-or-rust/README.md) had to add a minimal `credit_lines` before either implementation could run the hot path at all. **The card flow's decision step is not implementable against the committed schema** — the hold *log* is built, the credit line it is decided against is not. |
