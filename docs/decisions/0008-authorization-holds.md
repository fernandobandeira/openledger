...
adapter, a writer or a sweep, **that code does not exist**: `src/main.rs` prints "nothing to run yet"
and nothing in the tree reads or writes these tables.
...
| **The job key REPLACES; it does not collapse** | `JobKeyMode` defaults to `Replace`, so a second enqueue under the same key **overwrites the first, including its `run_at`**. Three enqueues left one row — and moved the timer two days. A wrapper shaped like `at(tx, kind, key, when)` re-enqueued with an earlier `when` moves an expiry *earlier*, and `held_minor` is `GENERATED` to 0 the instant `expired_at` is set, so a premature expiry **under-reserves the whole hold**. `UnsafeDedupe` is the mode that behaves as "idempotent". An earlier version of this row said the duplicate was collapsed silently and that this was the one ergonomic given up; both halves were wrong — `add_raw_job` returns `revision`, which carries the same signal River's `UniqueSkippedAsDuplicate` does. |
