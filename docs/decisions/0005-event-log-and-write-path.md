...
([0001](./0001-rust-and-postgres.md)).
...
- **None of this binds direct DML.** A posting type is a guarantee about our code, and says nothing
  ...
  That is what the two triggers in [`schema/schema.sql`](../../schema/schema.sql) are for, and why
  [0004](./0004-where-logic-lives.md) keeps them: they are the outer layer, and they bind accidents,
  not intent.
...
**global** contention point, so it defeats every throughput lever [0002](./0002-scaling.md) depends
...
rule against configurable correctness. Decide it alongside [0006](./0006-time-and-as-of.md), since a
