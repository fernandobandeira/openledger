...
pub async fn run(database_url: &str, lock_wait: Duration) -> Result<(), MigrateError> {
    let options: PgConnectOptions = database_url
        .parse()
        .map_err(|e| MigrateError::Usage(format!("could not parse the database URL: {e}")))?;
    ...
    let mut conn = PgConnection::connect_with(&options.application_name("openledger-migrate"))
        .await
        .map_err(|e| MigrateError::Failed(format!("could not connect: {e}")))?;

    conn.execute(SET_LOCK_TIMEOUT)
        .await
        .map_err(|e| MigrateError::Failed(format!("could not set lock_timeout: {e}")))?;

    if !acquire_lock(&mut conn, lock_wait).await? {
        return Err(MigrateError::Locked(format!(
...
async fn apply(conn: &mut PgConnection) -> Result<(), MigrateError> {
    ...
    // Elapsed-ms for the success line below: operator-facing monotonic
    // timing in a job's log, not ledger time (clippy.toml disallows ambient
    // clocks workspace-wide; `recorded_at` and `effective_at` are the only
    // clocks a posting sees).
    #[expect(
        clippy::disallowed_methods,
        reason = "operator-facing elapsed time in the migrate log, not ledger time"
    )]
...
fn classify(error: sqlx::migrate::MigrateError) -> MigrateError {
    ...
    if blocked {
        ...
        MigrateError::Failed(format!("migration failed: {error}"))
    }
}
...
async fn acquire_lock(conn: &mut PgConnection, budget: Duration) -> Result<bool, MigrateError> {
    // The wait deadline is operator-facing monotonic timing — the budget a
    // human chose on the command line — not ledger time, which is why these
    // two reads carry an `#[expect]` against clippy.toml's ambient-clock ban.
    #[expect(
        clippy::disallowed_methods,
        reason = "the lock-wait budget is operator-facing monotonic timing, not ledger time"
    )]
    let deadline = Instant::now()
        .checked_add(budget)
        .ok_or_else(|| MigrateError::Usage("the lock budget is too large".to_owned()))?;
    ...
    loop {
        let (acquired,): (bool,) = sqlx::query_as("SELECT pg_try_advisory_lock($1)")
            ...
            .fetch_one(&mut *conn)
            .await
            .map_err(|e| MigrateError::Failed(format!("could not take the migration lock: {e}")))?;
...
#[cfg(test)]
mod tests {
    //! Rules ADR-0003 states about the migration FILES, held against the
    //! set `sqlx::migrate!` compiles into this binary — the macro would
    //! accept either violation without comment, which is why each rule is
    //! a test here rather than only a sentence in the ADR. No database in
    //! the room: how `migrate` RUNS them (the try-lock, the timeout
    //! classification) is exercised end to end by the e2e suite.
